...
class ServerCard {
  ...
  static create(region, data) {
    const relaysCount =
      data.relays && Array.isArray(data.relays) ? data.relays.length : 0;
    const card = document.createElement("div");
    card.className =
      "server-card backdrop-blur-md bg-opacity-10 bg-white rounded-xl p-6 transform transition-all duration-300 hover:scale-105 border border-green-500/20";
    card.dataset.region = region;
    card.title = `${data.desc || region}\n${relaysCount} servers available`;
    ...
    flag.textContent =
      this.REGION_FLAGS[region] ||
      this.FLAGS[this.getCountryForRegion(region)] ||
      "🌐";
    ...
    if (
      data.relays &&
      data.relays.some(() => ServerManager.isBlocked(region))
    ) {
      const blockedBadge = document.createElement("span");
      blockedBadge.className =
        "px-2 py-1 text-xs font-semibold rounded-full bg-red-500/20 text-red-400";
    ...
    toggleBtn.className =
      "w-full mt-4 px-6 py-3 rounded-lg transition-all duration-300 flex items-center justify-center gap-2 font-medium";
    this.updateButtonState(toggleBtn, region, relaysCount);
    toggleBtn.onclick = () =>
      relaysCount > 0 ? his.toggleServer(region, data, toggleBtn) : null;
  ...
  static updateButtonState(button, region, relaysCount) {
    const isBlocked = ServerManager.isBlocked(region),
      hasRelays = relaysCount > 0;
    ...
    try {
      ...
      if (ServerManager.isBlocked(region)) {
        if (!existingBadge) {
          const blockedBadge = document.createElement("span");
          blockedBadge.className =
            "blocked-badge px-2 py-1 text-xs font-semibold rounded-full bg-red-500/20 text-red-400";
          blockedBadge.textContent = "🔒 Blocked";
          headerLeft.appendChild(blockedBadge);
        }
